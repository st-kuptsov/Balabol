...
OLD_IMAGE=$(docker images -q balabol:latest)
if [ -n "$OLD_IMAGE" ]; then
  echo "Удаляем старый образ balabol:latest..."
  docker rmi "$OLD_IMAGE"
else
  echo "Образ balabol:latest не найден, пропускаем удаление."
fi
